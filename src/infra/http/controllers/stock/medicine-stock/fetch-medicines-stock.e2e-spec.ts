...
describe('Fetch Medicines On Stock (E2E)', () => {
  ...
  test('[GET] /medicine-stock', async () => {
    ...
    const [pharmaceuticalForm1, pharmaceuticalForm2] = await Promise.all([
      pharmaceuticalFormFactory.makePrismaPharmaceuticalForm({ content: 'comp' }),
      pharmaceuticalFormFactory.makePrismaPharmaceuticalForm({ content: 'pill' }),
    ])

    const [medicineVariant1, medicineVariant2, medicineVariant3, medicineVariant4] = await Promise.all([
