...
describe('Reverse Exit (E2E)', () => {
  ...
  test('[POST] /moviment/exit/:id/reverse', async () => {
    ...
    const response = await request(app.getHttpServer())
      .post(`/movement/exit/${exit1.id.toString()}/reverse`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send();

    const [medicinesOnStock, batchesOnStock, medicinesEntered, movimentations] =
      await prisma.$transaction([
        prisma.medicineStock.findUnique({
          where: {
            id: medicineStock.id.toString(),
          },
        }),
        prisma.batcheStock.findUnique({
          where: {
    ...
    expect(medicinesOnStock?.currentQuantity).toBe(50);
    expect(batchesOnStock?.currentQuantity).toBe(50);
