...
describe('Get Exit Details (E2E)', () => {
  ...
  test('[GET] /exit/:id', async () => {
    ...
    const exit = await medicineExitFactory.makePrismaMedicineExit({
    ...
    const response = await request(app.getHttpServer())
      .get('/exit/' + exit.id.toString())
      .set('Authorization', `Bearer ${accessToken}`)
      .send();

    expect(response.statusCode).toBe(200);
    expect(response.body.exitDetails).toMatchObject({
      id: exit.id.toString(),
    });
  });
});
