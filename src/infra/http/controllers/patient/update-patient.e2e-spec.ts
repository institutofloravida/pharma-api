...
describe('Update Patient (E2E)', () => {
  ...
  test('[PUT] /patient/:id', async () => {
    ...
    const pathology2 = await pathologyFactory.makePrismaPathology()
    ...
    const response = await request(app.getHttpServer())
      .put(`/patient/${patient.id.toString()}`)
      ...
      .send({
        ...
        cpf: '01234567891',
        gender: 'M',
        pathologiesIds: [pathology.id.toString(), pathology2.id.toString()],
        race: 'MIXED',
        sus: '1234567890123456',
        addressPatient: {
          street: 'rua do centro',
    ...
    if (patientOnDataBase) {
      expect(patientOnDataBase.pathologies).toHaveLength(2)
    }
