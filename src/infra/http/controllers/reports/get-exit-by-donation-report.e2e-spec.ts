...
describe('Get Donation Report (E2E)', () => {
  ...
  test('[GET] /reports/movimentation', async () => {
    const date = new Date(2025, 0, 1)
    vi.setSystemTime(date)

    const institution = await institutionFactory.makePrismaInstitution()
    const destinationInstitution = await institutionFactory.makePrismaInstitution()
    ...
    expect(response.body.movimentation).toHaveLength(2)
  })
})
