...
import { GetExitByDonationUseCase } from './get-exit-by-donation-report'
...
let sut: GetExitByDonationUseCase
describe('Get exit by donation', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    ...
    sut = new GetExitByDonationUseCase(
      inMemoryMedicinesExitsRepository, inMemoryMovimentationRepository,
    )
  })

  it('should be able to get exit by donation details', async () => {
    ...
    const exit1 = makeMedicineExit({
      ...
      destinationInstitutionId: institution.id,
    })
    await Promise.all([
