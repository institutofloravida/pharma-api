...
import { DeleteStockUseCase } from './delete-stock';
...
describe('Delete Stock', () => {
  beforeEach(() => {
    ...
    sut = new DeleteStockUseCase(
      inMemoryStocksRepository,
      inMemoryMedicinesEntriesRepository,
      inMemoryMedicinesExitsRepository,
    );
  });
  it('should be able delete a stock', async () => {
  ...
  it('should not be able delete a stock that has movimentations', async () => {
    const institution = makeInstitution();
    await inMemoryInstitutionsRepository.create(institution);
