...
export class DeleteMedicineVariantUseCase {
  constructor(private medicineVariantsRepository: MedicinesVariantsRepository,
    private medicinesStocksRepository: MedicinesStockRepository
) {}

  async execute({ medicineVariantId }: deleteMedicineVariantUseCaseRequest): Promise<deleteMedicineVariantUseCaseResponse> {
    const medicinevariant = await this.medicineVariantsRepository.findById(medicineVariantId)
    if (!medicinevariant) {
    ...
    const {
      meta,
    } = await this.medicinesStocksRepository.findMany({page: 1 }, {medicineVariantId: medicinevariant.id.toString()})
