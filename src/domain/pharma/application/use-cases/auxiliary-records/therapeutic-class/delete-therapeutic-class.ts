...
export class DeleteTherapeuticClassUseCase {
  ...
  async execute({ therapeuticClassId }: deleteTherapeuticClassUseCaseRequest): Promise<deleteTherapeuticClassUseCaseResponse> {
    ...
    const {
      meta,
    } = await this.medicinesRepository.findMany({ page: 1 }, { therapeuticClassesIds: [therapeuticclass.id.toString()] })
