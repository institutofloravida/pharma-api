...
export class RegisterExitUseCase {
  ...
  async execute({
    ...
    if (batches.length <= 0) {
      return left(
        new AtLeastOneMustBePopulatedError(
          'É necessário ter pelo menos um lote para efetuar a saída ',
        ),
      );
    }
    ...
    if (destinationInstitutionId && exitType !== ExitType.DONATION) {
      return left(
        new Error(
          'A instituição de destino só pode ser informada quando o tipo de saída for DOAÇÃO',
        ),
      );
    }
    ...
    for await (const itemBatch of batches) {
      const batchestock = await this.batchesStocksRepository.findById(
