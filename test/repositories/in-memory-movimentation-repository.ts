...
implements MovimentationRepository {
  ...
  public setEntriesRepository(
    repo: InMemoryMedicinesEntriesRepository,
  ): void {
    this.entriesRepository = repo
  }

  public setExitsRepository(
    repo: InMemoryMedicinesExitsRepository,
  ): void {
  ...
  async fetchMovimentation(filters: {
    ...
    for (const movimentation of this.items) {
      ...
      if (movimentation.direction === 'ENTRY') {
        if (!movimentation.entryId) {
          throw new Error('Movimentation entryId is required for ENTRY direction.')
        }
      ...
      if (movimentation.direction === 'EXIT') {
        if (!movimentation.exitId) {
          throw new Error('Movimentation exitId is required for EXIT direction.')
        }

        movement = this.getExitsRepository().items.find((exit) =>
          exit.id.equal(
            movimentation.exitId
              ? movimentation.exitId
              : new UniqueEntityId(),
          ),
        )
      ...
      if (exitType && movimentation.direction === 'EXIT' && exitType !== movement.exitType) {
        continue
      }
      ...
      const medicineExitDetails = MovimentationDetails.create({
        direction: 'EXIT',
        ...
        exitType: movimentation.direction === 'EXIT'
          ? movement.exitType
          : undefined,
        movementDate: movimentation.direction === 'ENTRY'
          ? movement.entryDate
          : movement.exitDate,
    ...
    return {
      movimentation: exitsMovimentationFilteredAndMapped,
      meta: {
